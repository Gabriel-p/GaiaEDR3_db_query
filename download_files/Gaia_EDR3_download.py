...
def main(mag_max=20, dest_folder='datafiles'):
    """
    Download Gaia EDR3 source data files.
    ...
    gaia_files_file = "Gaia_EDR3_files.txt"
    ...
    if os.path.exists(gaia_files_file):
        ...
        print("File with Gaia EDR3 list of files read")
    else:
        # Generate file
        ext = 'gz'
        ...
        with open(gaia_files_file, 'w') as file:
            for line in gaia_files:
                file.write(line + '\n')
        print("File with Gaia EDR3 list of files generated")
    ...
    for file_url in gaia_files:
        ...
        try:
            with gzip.open(file_path) as f:
                data = pd.read_csv(f, usecols=req_cols, index_col=False)
